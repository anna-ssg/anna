...
- `{{.DeepDataMerge.CollectionsMap}}` - A map that stores a slice of templates of all posts for a particular collection url
...
- `type`:
  ...
  - `type: post` pages are added to the Posts field to be used via DeepDataMerge
  - Use type `page` for regular pages, `post` for posts & `note` for notes
