...
(
    ...
    go build -o anna
    GOEXPERIMENT=greenteagc go build -o anna_greentea
)
...
hyperfine -p 'sync' -w "$warm" \
  ...
  "cd $BENCH_DIR/anna-release && ./anna" \
  "cd $BENCH_DIR/anna-main && ./anna_greentea" \
  "cd $BENCH_DIR/anna-main && ./anna" \
  "cd $BENCH_DIR/anna && ./anna_greentea" \
  "cd $BENCH_DIR/anna && ./anna" \
  "cd $BENCH_DIR/hugo && hugo" \
